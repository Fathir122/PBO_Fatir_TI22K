...
class FormLogin:
    ...
    def aturKomponen(self):
        ...
        Label(mainFrame, text='Level:').grid(row=2, column=0, sticky=W, padx=5, pady=5)

        self.cmbLevel = ttk.Combobox(mainFrame, values=["Admin", "Mahasiswa", "Dosen"])
        self.cmbLevel.grid(row=2, column=1, padx=5, pady=5)
        
        # Button
        self.btnSubmit = Button(mainFrame, text='Submit', command=self.onSubmit, width=10)
        self.btnSubmit.grid(row=3, column=0, padx=5, pady=5)
        self.btnCancel = Button(mainFrame, text='Cancel', command=self.onKeluar, width=10)
        self.btnCancel.grid(row=3, column=1, padx=5, pady=5)
      
                 
    def onSubmit(self, event=None):
        email = self.txtEmail.get()
        password = self.txtPassword.get()
        level = self.cmbLevel.get()
    ...
    def open_warnet(self):
        title = "FrmWarnet"

        warnet = FormWarnet(self.parent, title)
